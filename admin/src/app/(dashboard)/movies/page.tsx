...
import { Plus, Film, Edit, Eye, Trash2, CheckCircle2, Archive, Star, RefreshCw } from "lucide-react";
...
import { useMoviesQuery, MovieRecord } from "@/hooks/useAdminQueries";
...
export default function MoviesManagementPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card/90 p-6 rounded-2xl border border-border/80 shadow-md backdrop-blur-md">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Manage movie listings, release schedules, genres, languages, cast metadata, and box office earnings.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Can permission="movie:create">
            <a href="/movies/new">
              <Button size="sm" className="h-9 text-xs font-bold gap-1.5 shadow-md">
      ...
      {/* Movies Table */}
      <DataTable columns={columns} data={moviesList} searchKey="title" searchPlaceholder="Search movies by title..." isLoading={isLoading} />
    </div>
  );
}
